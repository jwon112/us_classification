...
class EnhancedConvNeXt(nn.Module):
    def __init__(self, num_classes):
        super().__init__()
        # ConvNeXt-Tiny 백본 사용 (가장 작은 버전)
        convnext = models.convnext_tiny(weights=None)
        
        # ConvNeXt의 feature extractor 사용 (classifier 제외)
        self.features = convnext.features
        
        # ConvNeXt-Tiny의 실제 출력 채널 수 감지
        # 테스트 입력으로 채널 수 확인
        with torch.no_grad():
            test_input = torch.randn(1, 3, 224, 224)
            test_output = self.features(test_input)
            actual_channels = test_output.shape[1]
            print(f"ConvNeXt actual output channels: {actual_channels}")
    ...
    def forward(self, x):
        x = self.features(x)  # ConvNeXt feature extraction
